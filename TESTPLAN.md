...
- **Phase 0 census** in every scenario: reflection sweep of
  ...
  - ≥ 7 patched methods (forced-dry fall-through ×2, the ranged-selection scope ×3, armor-aware melee, drafted top-off; reload-abort is a
  GameComponent and has no census line) - plus a startup-error sweep of the
  baselined log.
- **Module isolation**: the shared test env loads the sibling Loadouts module,
  which patches the same SS selection methods, so the runner Harmony-unpatches it
  by owner id (`eebette.CESimpleSidearmsCompat.Loadouts`) before the scenarios run -
  decoupled from Loadouts' internals, so its redesign cannot contaminate the board.
  The core compat patch
  stays LOADED: Tactics declares it a dependency, and its RangedDpsCE/ReloadGuard/MeleePenetrationCE behavior
...
re-click, save/load stamps, under-barrel switches), the ranged-selection scope↔AmmoAwareSelection postfix
...
- **tact1 (reload-abort)**: feature-off reload completes; feature-on
  ...
  (T3-1 - unfixed, reload-abort killed the top-off and juggled weapons forever).
- **tact2 (forced-dry fall-through)**: feature-off forced branch holds a dry revolver;
...
- **tact3 (ammo-depth tiebreak)**: equal-DPS twins resolve to the
  deeper-magazine twin; a clearly better rifle with 1 round still wins
  (the tie window stays subordinate to DPS).
- **tact4 (target-aware ammo scoring)**: at 8 cells the buckshot
...
- **tact5 (armor-aware melee)**: knife beats mace vs flesh and the mace
...
- **tact6 (drafted sidearm top-off, drafted top-off - shares TACT-1's save)**: feature-off
...
1. **v0.1's first real bug (reload-abort design)**: asking SS's `findBestRangedWeapon`
...
2. **Uniform penetration floor erased blunt (armor-aware melee, v0.1 - superseded)**: the
   ...
   the "flesh" target.
3. **Core-patch RangedDpsCE interplay (T1's headline)**: the tact1 swap phases parked
   ...
   weapons 0, so reload-abort now (correctly) declines the swap and finishes the
...
6. **armor-aware melee's factorization matters.** SS's biased melee score is
   ...
   still let the mace beat the knife vs bare flesh. armor-aware melee divides the (1+pen)
   term out (StatCalculator.MeleePenetration, the same MeleePenetrationCE-backed input SS
...
7. **All-zero means stand down, not re-rank.** Against centipede plate every
   ...
   Both ranged-selection scope and armor-aware melee now defer to SS's own pick when no candidate scores
   positive: re-ranking zeros is noise. Pinned by tact4's
   `on-hopeless-armor-defers-to-raw` and tact5's `on-vs-armor-picks-blunt`.
8. **The knife-vs-flesh red unmasked a CORE-PATCH gap (now MeleeDamageCE).** SS's melee
...
9. **ranged-selection scope's first shape was the transcription the rules ban (reworked).** Its
   ...
   opened by a prefix on findBestRangedWeapon (retrieval scope's pattern, closed by a
   ...
   automatically. RangedDpsCE's cache is untouched (it stores in its prefix, before
...
11. **Isolated runs exposed hidden phase coupling**: tact2/tact3 later phases
   ...
   patch's MeleePenetrationCE alone). Every phase now re-establishes its own state in
...
12. **T3 (2026-09-01), the adversarial round's own ledger.** Three independent
   ...
   - **T3-1** reload-abort identified "reloading" with "the equipped gun is empty" -
     CE's undrafted top-offs and drafted top-off's drafted top-offs falsified it; the
     abort now touches only reloads of the gun in hand.
   - **T3-2** armor-aware melee's target parameter was dead wiring (SS never forwards the
     CQC target into findBestMeleeWeapon); reworked to the ranged-selection-scope pattern
   ...
   - **T3-4** the all-hopeless defer resurrected DRY guns (records carry
     paper scores; AmmoAwareSelection's correction had already run); the defer now filters
     to guns with rounds.
   - **T3-5** reload-abort's winner scan lacked SS's usability rule (biocode/bond/
   ...
   - Accepted + documented: **T3-7** (a drafted melee-primary pawn topping
     ...
     as issue #4/future feature candidate) and **T3-9** (CE's reload driver kills ANY
     reload on a primary change, backpack ones included - the compat ReloadGuard
     ...
     "hypothetical" preference question into real equips. **T3-10**: reload-abort
...
- **C2**: pair-level dryness judged by the first carried instance - a drained
  ...
  pair is not hidden).
- **C3 (owner ruling: no mirrored filters)**: reload-abort's winner scan was still a
  ...
  loaded-this-instant filter on GetCarriedWeapons (AmmoAwareSelection's own pattern) - SS's
  full chain and ranged-selection scope's target-aware scoring inherited, census 11. Pinned by
...
- **TargetScoring rework (owner rulings: A + C)**: the per-hit arithmetic is
  ...
  TryPenetrateArmor, GetDeflectDamageInfo, and drafted top-off's DoReloadCheck: any
...
- Also: Bootstrap counts Prepare-false classes as skipped, not applied; reload-abort
...
T4-3's fallback alignment, the fingerprints, Bootstrap, and the reload-abort gate
...
- **T5-D (Low)**: ranged-selection scope's findBestRangedWeapon postfix composed AFTER the core
  patch's AmmoAwareSelection dry-pick re-run - when SS's pick was truly dry, AmmoAwareSelection's
  overwrite (whose inner call is already fully ranged-selection scope-processed) got
  re-processed by the outer ranged-selection scope postfix against the outer records with the
  ...
  .HarmonyId)]` on the postfix - AmmoAwareSelection's overwrite now runs last and discards
...
- **T4-2 (High)**: reload-abort's player-respect gate read `job.playerForced` - but
...
- **T4-3 (Low, moot)**: the modeled FALLBACK deflected zero-pen sharp attacks
  differently from CE; it was aligned, then the fallback was removed entirely -
  TargetScoring now degrades to unmodelable when CE's TryPenetrateArmor cannot
  bind, rather than reproduce CE's armor math.
- **T4-4 (Low)**: fingerprint operand tokens used culture-sensitive
  ...
  permanently red the suite. Both copies (Tactics + compat's WarmupAutoSwitchCE guard) now
...
- **T4-3..T4-6**: load-time machinery - covered by
...
- **T3-2** (melee scope): all three tact5 target phases red; census stays
  green ON PURPOSE - forced-dry fall-through still patches the same method, so the count holds
...
- **T3-11** (refill-in-flight): red only through the MELEE-override preference
  ...
  ReloadGuard guard, and the first version of the leg accidentally pinned ReloadGuard instead.
...
- **reload-abort neutered** (component tick short-circuited): tact1
  ...
  component features.
- **forced-dry fall-through Prepare→false**: tact2 census red (3 < 4 methods) AND
  `on-falls-through-to-pistol` red - one run, both detection layers.
- **Core MeleeDamageCE Prepare→false** (cross-repo): compat census red (22 < 23),
...
- **ranged-selection scope zero-defer removed** (pre-rework note, kept for history): came back
...
- **ranged-selection scope Prepare→false** (the rework's leg): census red (6 < 7)
...
- **drafted top-off Prepare→false**: census red (6 < 7) AND tact6's lull top-off red - the
  sidearm stays empty through the lull with the patch gone.
- **armor-aware melee de-bias removed**: tact5 `on-vs-flesh-picks-blade` red (the generic
